...
function listStructs() {
	...
	for ( var i in structListItems) {
		...
		var oid = $j("a", structListItems[i]).attr("oid").toLowerCase();
		if (structName.indexOf(searchStr) >= 0
				|| oid.indexOf(searchStr) >= 0) {
			$j("#structList").append(structListItems[i]);
		}
	}
}
...
function loadStructList() {
	...
	$j("#structList").empty();
	structListItems = [];
	snmpStructService.getAllStructOidsAndNames($j("#projectId").val(), function(structs) {
		console.log(structs);
		$j("#spanStructCount").text("[" + structs.length + "]");
		structList = structs;
		for ( var i in structs) {
			var structName = structs[i].structName;
			var oid = structs[i].oid;
			var displayName = structName;
			if ("" == displayName) {
				displayName = "[empty]";
			}
			var item = $j("<li><a title=\"" + structName + "\" oid=\"" + oid + "\" href=\"javascript:void(0)\" onclick=\"showStruct('" + structName
					+ "')\">" + displayName + "</a></li>");
...
function showCheckPointDlg(){
	...
	checkPointService.getAllCheckPoints(projectId, "snmp", function(checkpoints){
...
function createCheckPoint(){
	...
	snmpStructService.createCheckPoint(projectId, description, function(checkpoint){
		...
		buttons : [
			{
				...
				click : function(){
					var waittingDlg = showWaitingDlg();
					snmpStructService.checkPointRecovery(checkpointId, function(result){
						waittingDlg.dialog("close");
